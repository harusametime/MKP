...
class runBranchAndBound extends Algorithm {
	...
	void run() {
		...
		double best_eval = 0; // 暫定値
		SearchTree tree = new SearchTree();
		int max = Bags.size();
		double infeasible_lb = -1;
		...
		for (int i = 0; i < Golds.size(); i++) {
			...
			// 制約を満足していれば下界値を更新する。
			if (!LB.setMatrix(tree, Bags, Golds)) {
				double lb = LB.value();
				newnode = tree.pop();
				newnode.lb = lb;
				tree.push(newnode);
		...
		while (!tree.empty()) {
			...
			if (tree.size() == Golds.size() || tree.peek().lb == -1
...
class LowerBound {
	...
	public boolean setMatrix(SearchTree tree, ArrayList<Bag> Bags,
		...
		for (int j = 0; j < Golds.size(); j++) {
			...
			if(flag){
				if (tree.get(x).bag > 0) {
				b[tree.get(x).bag-1] -= Golds.get(tree.get(x).gold).weight;
				if (b[tree.get(x).bag-1] < 0) {
					return true;
					}
				}
				...
				constant += Golds.get(tree.get(x).gold).value;
			}else {
					for (int i = 0; i < Bags.size(); i++) {
						A[i][i * Golds.size() + j] = Golds.get(j).weight;
						c[i * Golds.size() + j] = Golds.get(j).value;
					}
...
/*		for (int j = 0; j < Golds.size(); j++) {
			for (int k = 0; k < tree.size(); k++) {
				if (j == tree.get(k).gold) {
					if (tree.get(k).bag > 0) {
						b[tree.get(k).bag - 1] -= Golds.get(tree.get(k).gold).weight;
						if (b[tree.get(k).bag - 1] < 0) {
							return true;
						}
						for (int i = 0; i < Bags.size(); i++) {
							if (i == tree.get(k).bag - 1) {
								A[i][i * Golds.size() + j] = 0;
								A[j+Bags.size()][i * Golds.size() + j] = 1;
								c[i * Golds.size() + j] = 0;
							} else {
						...
						constant += Golds.get(tree.get(k).gold).value;
					} else {
						for (int i = 0; i < Bags.size(); i++) {
							if (i == tree.get(k).bag - 1) {
								A[i][i * Golds.size() + j] = 0;
